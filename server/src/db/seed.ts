...
function seedAll(app: DatabaseSync, identity: DatabaseSync, crypto: FieldCrypto): void {
  ...
  app.prepare('INSERT INTO eval_run (id,model_release_id,eval_set_id,metrics,result,trigger_reason,created_at) VALUES (?,?,?,?,?,?,?)')
    .run(uid(), mr1, es1, JSON.stringify({ 错误安慰: 0, 关键遗漏: 0, 左右侧混淆: 0, 隐私: 0, 用例通过率: 1 }), '通过', '发布前门禁', t);
