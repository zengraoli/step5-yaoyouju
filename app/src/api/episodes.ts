...
/** 病程时间线（服务端按北京日期分组，含症状日志与报告术语） */
export function getEpisodeTimeline(episodeId: string): Promise<{ date: string; events: CareEventView[] }[]> {
  return request<{ date: string; events: CareEventView[] }[]>({
    url: `/episodes/${encodeURIComponent(episodeId)}/timeline`,
  })
}

/** 记录今天（允许跳过；缺失字段服务端记为「尚未确认」，不复用昨日答案） */
